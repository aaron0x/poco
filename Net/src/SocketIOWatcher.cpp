...
// SocketIOWatcher.cpp
//
// $Id: //poco/1.4/Net/src/SocketIOWatcher.cpp#1 $
//
// Library: Net
// Package: Sockets
...
int SocketIOWatcher::select(SocketList& readList, SocketList& writeList, SocketList& exceptList, const Poco::Timespan& timeout)
{
#if defined(POCO_HAVE_FD_EPOLL)
...
bool SocketIOWatcher::poll(SocketImpl* socketImpl, const Poco::Timespan& timeout, int mode)
{
   poco_check_ptr(socketImpl);

   if (socketImpl->sockfd() == POCO_INVALID_SOCKET) throw InvalidSocketException();

   Socket socket(socketImpl);
   socketImpl->duplicate();
   SocketList readList;
   SocketList writeList;
   SocketList exceptList;

   if (mode & SocketImpl::SELECT_READ)
      readList.push_back(socket);
   if (mode & SocketImpl::SELECT_WRITE)
      writeList.push_back(socket);
   if (mode & SocketImpl::SELECT_ERROR)
      exceptList.push_back(socket);

   return select(readList, writeList, exceptList, timeout) > 0;
}
